        ...
        <div className="pt-6 sm:pt-8 grid grid-cols-2 sm:grid-cols-4 gap-3 text-center border-t border-brand-cream/15 max-w-3xl mx-auto">
          {hero.features.map((feature, idx) => (
            <div key={idx} className="p-2">
              <p className="text-brand-cream text-xs sm:text-sm font-serif-title font-medium">{feature.title}</p>
              <p className="text-[11px] text-gray-400 mt-0.5">{feature.subtitle}</p>
            </div>
          ))}
        </div>
      </div>
