...
export const GallerySection: React.FC<GallerySectionProps> = ({ onOpenLightbox }) => {
  ...
  const filterTabs = [
    ...
    { id: 'interior', label: 'Ruang Dalam' },
    { id: 'craft', label: 'Artisan Barista' },
    { id: 'garden', label: 'Outdoor Garden' },
    { id: 'night', label: 'Suasana Malam' },
  ];
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
          {filteredPhotos.map((photo) => (
            <div
              ...
              className={`relative h-72 sm:h-80 rounded-3xl overflow-hidden cursor-pointer group shadow-xs hover:shadow-xl transition-all duration-300 ${photo.spanClass || ''}`}
            >
              <img
                src={photo.url}
                alt={photo.title}
