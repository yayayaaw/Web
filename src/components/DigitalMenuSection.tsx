...
export const DigitalMenuSection: React.FC<DigitalMenuSectionProps> = ({
  ...
  const [activeFilter, setActiveFilter] = useState<'all' | 'popular' | 'vegetarian'>('all');
  ...
  const categories: { id: MenuCategory; label: string }[] = [
    ...
    { id: 'main', label: 'Makanan Utama' },
    { id: 'snack', label: 'Camilan Gurih' },
    { id: 'dessert', label: 'Penutup Manis' },
  ];
  ...
  const filteredItems = useMemo(() => {
    return MENU_ITEMS.filter((item) => {
      ...
      if (activeFilter === 'vegetarian' && !item.isVegetarian) {
        return false;
      }
      return true;
    });
    ...
    <section id="menu" className="py-20 md:py-32 bg-brand-cream/40 relative">
      <div className="max-w-7xl mx-auto px-5 sm:px-8 md:px-12">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
          ...
          {/* Quick catalog print / download button */}
