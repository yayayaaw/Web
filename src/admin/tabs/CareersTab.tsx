...
// ============ BAGIAN 1: HEADER HALAMAN KARIR ============

function CareersHeaderEditor() {
  const [form, setForm] = useState<CareersHeaderContent>(() => getContent().careersHeader);
  const [saved, setSaved] = useState(false);

  function handleSave(e: React.FormEvent) {
    e.preventDefault();
    setContent('careersHeader', form);
    setSaved(true);
  ...
  return (
    <div className="bg-white p-5 rounded-2xl border border-[#F4EFE6] shadow-sm space-y-4">
      <div>
        <h4 className="font-bold text-[#1A1A1A]">Header Halaman Karir</h4>
        <p className="text-xs text-gray-500">Banner paling atas di halaman #karir (eyebrow, judul, deskripsi).</p>
      </div>
      <form onSubmit={handleSave} className="space-y-3 text-xs">
        <div>
          <label className="block font-semibold mb-1">Eyebrow (label kecil di atas judul)</label>
          <input
            className="w-full px-4 py-3 rounded-xl border border-[#F4EFE6] bg-white outline-none"
        ...
        <button type="submit" className="px-5 py-2.5 rounded-xl bg-[#1A1A1A] text-white font-bold text-xs">
...
// ============ BAGIAN 2: BUDAYA & KEUNTUNGAN STAF (PERKS) ============

const PERK_ICONS: { value: CareerPerkIcon; label: string }[] = [
  { value: 'Coffee', label: 'Coffee (cangkir kopi)' },
  { value: 'GraduationCap', label: 'GraduationCap (topi wisuda)' },
  { value: 'Award', label: 'Award (medali)' },
  { value: 'Utensils', label: 'Utensils (sendok garpu)' },
  { value: 'ShieldCheck', label: 'ShieldCheck (perisai centang)' },
...
const EMPTY_PERK_FORM: PerkForm = {
  icon: 'Coffee',
  title: '',
  desc: '',
};

function CareerPerksEditor() {
  const [perks, setPerks] = useState<CareerPerkItem[]>([]);
  const [modalOpen, setModalOpen] = useState(false);
  const [editingId, setEditingId] = useState<string | null>(null);
  const [form, setForm] = useState<PerkForm>(EMPTY_PERK_FORM);

  useEffect(() => {
    setPerks(getContent().careerPerks);
  }, []);

  function persist(next: CareerPerkItem[]) {
    setPerks(next);
    setContent('careerPerks', next);
  }

  function openAddModal() {
    setEditingId(null);
    setForm(EMPTY_PERK_FORM);
    setModalOpen(true);
  }

  function openEditModal(perk: CareerPerkItem) {
    setEditingId(perk.id);
    setForm({ icon: perk.icon, title: perk.title, desc: perk.desc });
    setModalOpen(true);
  }

  function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    const payload: CareerPerkItem = {
      id: editingId ?? `perk-${Date.now()}`,
      icon: form.icon,
      title: form.title.trim(),
      desc: form.desc.trim(),
    };
    if (editingId) {
      persist(perks.map((p) => (p.id === editingId ? payload : p)));
    } else {
      persist([...perks, payload]);
    }
    setModalOpen(false);
    setForm(EMPTY_PERK_FORM);
    setEditingId(null);
  ...
  function handleDelete(id: string) {
    if (confirm('Hapus kartu keuntungan ini?')) persist(perks.filter((p) => p.id !== id));
  }
  ...
  return (
    <div className="bg-white p-5 rounded-2xl border border-[#F4EFE6] shadow-sm space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h4 className="font-bold text-[#1A1A1A]">Budaya & Keuntungan Staf</h4>
          <p className="text-xs text-gray-500">6 kartu perk di section "Mengapa Berkarir di Élysée Café?".</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        {perks.length === 0 && <p className="col-span-full text-center text-gray-400 py-4 text-xs">Belum ada kartu keuntungan.</p>}
        {perks.map((perk) => (
          <div key={perk.id} className="p-4 rounded-xl border border-[#F4EFE6] space-y-2">
            <div className="flex justify-between items-start gap-2">
              <div>
                <span className="text-[10px] font-semibold uppercase tracking-wide text-[#6F4E37] bg-[#6F4E37]/10 px-2 py-0.5 rounded-full">
                  {perk.icon}
                </span>
                <h5 className="font-bold text-[#1A1A1A] text-sm mt-1">{perk.title}</h5>
              </div>
              <div className="flex items-center gap-3 shrink-0">
                <button onClick={() => openEditModal(perk)} className="text-gray-500 hover:text-[#1A1A1A] text-xs"><i className="fa-solid fa-pen"></i></button>
                <button onClick={() => handleDelete(perk.id)} className="text-red-500 hover:text-red-700 text-xs"><i className="fa-solid fa-trash"></i></button>
              </div>
            </div>
            <p className="text-xs text-gray-600">{perk.desc}</p>
          </div>
        ))}
      </div>
      ...
      {modalOpen && (
        <div className="fixed inset-0 z-50 bg-black/60 flex items-start sm:items-center justify-center p-4 overflow-y-auto">
          <div className="bg-[#FDFBF7] rounded-3xl max-w-md w-full p-6 sm:p-8 space-y-6 relative border border-[#E6DEC8] my-8">
            ...
            <h3 className="text-2xl font-bold text-[#1A1A1A]" style={{ fontFamily: "'Cormorant Garamond', serif" }}>
              {editingId ? 'Edit Kartu Keuntungan' : 'Tambah Kartu Keuntungan'}
            </h3>
            <form onSubmit={handleSubmit} className="space-y-4 text-xs">
              <div>
                <label className="block font-semibold mb-1">Ikon</label>
                <select
                  className="w-full px-4 py-3 rounded-xl border border-[#F4EFE6] bg-white outline-none"
                  value={form.icon}
                  onChange={(e) => setForm({ ...form, icon: e.target.value as CareerPerkIcon })}
                >
                  {PERK_ICONS.map((opt) => (
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between bg-white p-4 rounded-2xl border border-[#F4EFE6] shadow-sm">
        <div>
          <h4 className="font-bold text-[#1A1A1A]">Lowongan Karir</h4>
            ...
            <div className="flex justify-between items-start gap-2">
              <div>
                <div className="flex items-center gap-2 flex-wrap">
                  <h5 className="font-bold text-[#1A1A1A]">{job.title}</h5>
                  {job.urgent && (
    ...
    <section className="space-y-6">
      <div>
        ...
        <p className="text-xs text-gray-500">
          Perubahan tersimpan ke browser & langsung tampil di web utama saat direfresh.
        </p>
      </div>
