...
import { getContent, setContent, MenuItemCard } from '../../lib/contentStore';
import { MenuModal } from '../modals/MenuModal';

export function MenuTab() {
  const [items, setItems] = useState<MenuItemCard[]>(() => getContent().menuItems);
  const [search, setSearch] = useState('');
  const [categoryFilter, setCategoryFilter] = useState('all');
  const [modalOpen, setModalOpen] = useState(false);
  const [editingItem, setEditingItem] = useState<MenuItemCard | null>(null);
  ...
  function persist(updated: MenuItemCard[]) {
    setItems(updated);
    setContent('menuItems', updated);
  }

  function openAdd() { setEditingItem(null); setModalOpen(true); }
  function openEdit(item: MenuItemCard) { setEditingItem(item); setModalOpen(true); }

  function handleSave(data: Omit<MenuItemCard, 'id'>) {
    if (editingItem) {
      persist(items.map(i => (i.id === editingItem.id ? { ...i, ...data } : i)));
    } else {
      persist([{ id: 'menu-' + Date.now(), ...data }, ...items]);
    }
    setModalOpen(false);
  }

  function handleDelete(id: string) {
    if (confirm('Hapus menu ini? Otomatis hilang juga dari halaman Menu Digital web utama.')) {
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-[#F4EFE6]/60 border-b border-[#F4EFE6] text-[11px] font-bold uppercase tracking-wider text-[#1A1A1A]">
                ...
                <th className="p-4">Harga</th><th className="p-4">Badge</th><th className="p-4 text-right">Aksi</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-[#F4EFE6] text-xs">
              {filteredItems.length === 0 && (
                <tr><td colSpan={6} className="p-6 text-center text-gray-400">Belum ada menu.</td></tr>
              )}
              {filteredItems.map(item => (
                <tr key={item.id} className="hover:bg-[#FDFBF7] transition-colors">
                  <td className="p-4"><img src={item.image} className="w-12 h-12 rounded-lg object-cover" /></td>
