...
//
// PATCH v3 (final, sesuai request "apus aja, gak usah ada filosofi kedua"):
// - AboutContent punya field `highlights` -- paragraf singkat di Our Story,
//   bisa diedit dari CMS (dulu hardcode & gak nyambung ke CMS).
// - Section "Filosofi Seduhan Presisi" (brewMethods) DIHAPUS TOTAL dari
//   model data ini -- bukan cuma diringkas jadi paragraf, tapi memang gak
//   dipakai lagi sama sekali, karena isinya duplikat dengan section
//   "Ketelitian dalam Tiap Seduhan" (philosophyCards) yang sudah ada di Home.
...
export interface AboutContent {
  ...
  subQuote: string;
  imageUrl: string;
  // Highlight singkat (paragraf, dulu kartu)
...
export function getContent(): SiteContent {
  try {
    ...
    return {
      ...
      // ke default -- jadi ga bakal blank/error di data lama. Field
      // `brewMethods*` yang mungkin tersisa dari patch sebelumnya di
      // localStorage otomatis diabaikan (bukan bagian dari AboutContent lagi).
      about: {
