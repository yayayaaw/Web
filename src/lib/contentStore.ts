...
//
// PATCH v6 (fix "Karir ga bisa diedit lengkap dari CMS"):
// Ditambah `jobPositions` -- lowongan karir yang tampil di CareersPage.
// Sebelumnya ini namanya JOB_POSITIONS, hardcode 100% di mockData.ts,
// gak ada jalur CMS sama sekali. Sekarang default-nya tetap ambil dari
// JOB_POSITIONS (biar konsisten sama data awal), tapi begitu diedit dari
// CMS, tersimpan ke localStorage dan itu yang dipakai web utama.
...
export interface SiteContent {
  ...
  jobPositions: JobPosition[]; // PATCH v6
  event: EventContent;
  about: AboutContent;
...
export const DEFAULT_CONTENT: SiteContent = {
  ...
  jobPositions: JOB_POSITIONS, // PATCH v6: default = data lowongan asli
  event: {
...
export function getContent(): SiteContent {
  try {
    ...
    return {
      ...
      jobPositions: parsed.jobPositions ?? DEFAULT_CONTENT.jobPositions, // PATCH v6
