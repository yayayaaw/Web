...
//
// PATCH (fix "Our Story ga sesuai data utama"):
// AboutContent sekarang punya field `highlights` -- 4 kartu (Biji Kopi Terpilih,
// Bahan Segar & Alami, dst) yang SEBELUMNYA hardcode di AboutSection.tsx dan
// TIDAK PERNAH terhubung ke CMS sama sekali (makanya cuma foto & paragraf atas
// yang keupdate, 4 kartu itu ga pernah berubah walau diedit dari admin).
// Sekarang 4 kartu itu juga tersimpan & bisa diedit lewat ContentPagesTab.
...
// Kunci ikon yang boleh dipilih admin di CMS untuk tiap kartu highlight.
// Pemetaan ke komponen Lucide asli ada di AboutSection.tsx (ICON_MAP).
...
export interface AboutContent {
  ...
  subQuote: string;
  imageUrl: string;
  // 4 kartu highlight (dulu hardcode, sekarang ikut CMS)
...
export const DEFAULT_CONTENT: SiteContent = {
  ...
  about: {
    ...
    highlights: [
      { id: 'ah-1', icon: 'coffee', title: 'Biji Kopi Terpilih', description: 'Biji kopi Arabika single-origin yang dipanggang secara presisi oleh roaster berpengalaman.' },
      { id: 'ah-2', icon: 'utensils', title: 'Bahan Segar & Alami', description: 'Setiap hidangan dimasak fresh dengan sayuran segar dan bahan organik suplier lokal.' },
      { id: 'ah-3', icon: 'armchair', title: 'Ruang Hangat & Nyaman', description: 'Tata letak ergonomis, pencahayaan temaram hangat, serta alunan musik santai akustik.' },
      { id: 'ah-4', icon: 'wifi', title: 'Koneksi Wi-Fi 100 Mbps', description: 'Dukungan internet cepat dan ketersediaan stopkontak di setiap sudut meja untuk produktivitas Anda.' },
    ],
  },
  contact: {
...
export function getContent(): SiteContent {
  try {
    ...
    return {
      ...
      // belum punya `highlights` (dibuat sebelum patch ini), otomatis fallback
      // ke default 4 kartu -- jadi ga bakal blank/error di web lama yang sudah kepakai.
      about: {
