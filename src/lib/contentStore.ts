...
//
// PATCH v8 (fix "tagline header hardcode" + "analytics kosong"):
// - Ditambah `header.tagline` -- teks kecil di sebelah nama brand (contoh:
//   "Café"). Sebelumnya ini hardcode literal "Café" langsung di Navbar.tsx,
//   gak ada jalur CMS sama sekali.
// - Ditambah `analytics` -- data dashboard "Analitik Pengunjung" di CMS.
//   Karena situs ini belum terhubung ke Google Analytics/Firebase (dan
//   memang sengaja tidak dipasang dulu), angka-angka ini diinput MANUAL
//   oleh admin lewat CMS, bukan tracking otomatis. Section ini cuma dipakai
//   di CMS (bukan tampil di web utama), jadi tidak perlu disentuh oleh
...
export interface HeaderContent {
  brandName: string;
  tagline: string; // PATCH v8: teks kecil di sebelah nama brand, contoh "Café"
}
...
// PATCH v5: foto "Élysée Moments" -- beda dari GalleryPhotoItem (galeri
// ruangan). Ini kotak-kotak kecil bergaya feed Instagram.
...
// Highlight singkat di bawah paragraf cerita -- tampil sebagai paragraf
// polos di web (bukan kartu berikon).
...
export interface AboutContent {
  ...
  subQuote: string;
  imageUrl: string;
  // Highlight singkat (paragraf, dulu kartu) -- list dinamis, bisa kosong
  highlights: AboutHighlight[];
}

// Banner/header paling atas di halaman #karir (PageHeader).
...
// Ikon dibatasi ke 6 pilihan ini karena CareersPage.tsx cuma punya
// switch-case buat 6 ikon lucide-react tersebut.
...
// ---------- PATCH v8: ANALYTICS (input manual, CMS-only) ----------

export interface AnalyticsTrendPoint {
  id: string;
  label: string; // contoh: "Sen", "Minggu 1"
  value: number;
}
...
export interface AnalyticsPeakHour {
  id: string;
  range: string; // contoh: "08:00 - 11:00"
  label: string; // contoh: "Morning Coffee & Pastry"
  percent: number;
}

export interface AnalyticsData {
  totalVisitors: number;
  totalVisitorsGrowthLabel: string; // contoh: "+12.4% bln ini"
  todayVisitors: number;
  todayVisitorsLabel: string; // contoh: "Aktif sesi terkini"
  weekVisitors: number;
  weekGrowthLabel: string; // contoh: "+8.2% vs lalu"
  monthVisitors: number;
  monthLabel: string; // contoh: "Agustus 2026"
  totalPageViews: number;
  avgViewsLabel: string; // contoh: "Rata-rata 3.1 view/tamu"
...
export interface SiteContent {
  ...
  analytics: AnalyticsData; // PATCH v8
  event: EventContent;
  about: AboutContent;
...
export const DEFAULT_CONTENT: SiteContent = {
  ...
  analytics: {
    totalVisitors: 1280,
    totalVisitorsGrowthLabel: '+0% bln ini',
    todayVisitors: 0,
    todayVisitorsLabel: 'Aktif sesi terkini',
    weekVisitors: 210,
    weekGrowthLabel: '+0% vs lalu',
    monthVisitors: 890,
    monthLabel: 'Bulan ini',
    totalPageViews: 3100,
    avgViewsLabel: 'Rata-rata 2.4 view/tamu',
    trend7Days: [
      { id: 't-1', label: 'Sen', value: 40 },
      { id: 't-2', label: 'Sel', value: 55 },
      { id: 't-3', label: 'Rab', value: 48 },
      { id: 't-4', label: 'Kam', value: 60 },
      { id: 't-5', label: 'Jum', value: 75 },
      { id: 't-6', label: 'Sab', value: 90 },
      { id: 't-7', label: 'Min', value: 82 },
    ],
    topSections: [
      { id: 'sec-1', name: 'Signature Menu', views: 1200, percent: 38 },
      { id: 'sec-2', name: 'Galeri & Sudut Ruang', views: 820, percent: 26 },
      { id: 'sec-3', name: 'Reservasi Meja', views: 600, percent: 19 },
      { id: 'sec-4', name: 'Lokasi & Kontak', views: 540, percent: 17 },
    ],
    deviceDistribution: {
      mobile: 72,
      desktop: 24,
    ...
    peakHours: [
      { id: 'ph-1', range: '08:00 – 11:00', label: 'Morning Coffee & Pastry', percent: 55 },
      { id: 'ph-2', range: '12:00 – 14:00', label: 'Lunch & Pour Over Break', percent: 70 },
      { id: 'ph-3', range: '15:00 – 18:00', label: 'Afternoon Work & Meetings', percent: 60 },
      { id: 'ph-4', range: '19:00 – 22:00', label: 'Evening Gathering & Chill', percent: 85 },
    ],
  },
  event: {
...
export function getContent(): SiteContent {
  try {
    ...
    return {
      ...
      // PATCH v8: merge dalam-dalam biar data lama (sebelum patch ini) yang
      // belum punya sub-field analytics otomatis fallback ke default.
      analytics: {
        ...DEFAULT_CONTENT.analytics,
        ...parsed.analytics,
        trend7Days: parsed.analytics?.trend7Days ?? DEFAULT_CONTENT.analytics.trend7Days,
        topSections: parsed.analytics?.topSections ?? DEFAULT_CONTENT.analytics.topSections,
        deviceDistribution: { ...DEFAULT_CONTENT.analytics.deviceDistribution, ...parsed.analytics?.deviceDistribution },
        peakHours: parsed.analytics?.peakHours ?? DEFAULT_CONTENT.analytics.peakHours,
      },
      event: { ...DEFAULT_CONTENT.event, ...parsed.event },
      // Merge dalam-dalam khusus 'about': kalau data lama di localStorage
      // belum punya `highlights` (dibuat sebelum patch), otomatis fallback
      // ke default -- jadi ga bakal blank/error di data lama.
      about: {
